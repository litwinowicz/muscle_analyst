...
def process_muscle_segmentation(
    ...
    """
    Process and analyze a muscle segmentation, with optional cropping based on reference segmentations.
    ...
    3. Full volume mode: Processes the entire volume without cropping (when no reference segmentations provided)

    Parameters
    ----------
    muscle_segmentation_path : str
        Path to the input muscle segmentation NIfTI file to be processed
    output_path : Optional[str]
        Path where the cropped muscle segmentation will be saved (only used in cropping modes)
    ...
    fat_fraction_output_path : Optional[str]
        Path where the fat fraction map will be saved as NIfTI file (only if IP/OOP provided)
    ...
    Dict[str, Union[tuple, float, int, Dict[str, float]]]
        ...
        - cropped_volume_cm3: Cropped volume in cm³ (only if cropping performed)
    ...
    ValueError
        ...
        If only one of start_seg_path or end_seg_path is provided
        If only one of ip_image_path or oop_image_path is provided
        If output_path is not provided when using cropping modes
    """
    ...
    # Determine processing mode and set crop points
    cropping_required = False
    
    if single_ref_seg_path is not None:
        if output_path is None:
            raise ValueError("output_path must be provided when using single reference mode")
        ...
        end_point = muscle_seg_data.shape[2] - 1  # Go to the end of the volume
        cropping_required = True
        
    elif start_seg_path is not None and end_seg_path is not None:
        if output_path is None:
            raise ValueError("output_path must be provided when using two-point mode")
    ...
    elif all(x is None for x in [single_ref_seg_path, start_seg_path, end_seg_path]):
        # Full volume mode - process entire volume without cropping
        start_point = 0
        end_point = muscle_seg_data.shape[2] - 1
        cropping_required = False
    else:
        raise ValueError(
            "Must provide either single_ref_seg_path for single-point cropping, "
            "both start_seg_path and end_seg_path for two-point cropping, "
            "or no reference segmentations for full volume processing"
        )
    ...
    # Process and save cropped volume if cropping is required
    if cropping_required:
        cropped_muscle_seg = muscle_seg_data[:, :, start_point:end_point + 1]
        cropped_volume = calculate_volume(cropped_muscle_seg, voxel_dims)
        result_dict['cropped_volume_cm3'] = cropped_volume / 1000
    ...
    if calculate_ff:
        if cropping_required:
            processed_ip = ip_data[:, :, start_point:end_point + 1]
            processed_oop = oop_data[:, :, start_point:end_point + 1]
            processed_seg = cropped_muscle_seg
        else:
            processed_ip = ip_data
            processed_oop = oop_data
            processed_seg = muscle_seg_data
        ...
        if fat_fraction_output_path:
            if cropping_required:
                ff_affine = new_affine
            else:
                ff_affine = muscle_seg_nii.affine
...
if hasattr(snakemake.input, "single_segmentation"):
    ...
    if hasattr(snakemake.input, "ip_image") and hasattr(snakemake.input, "oop_image"):
        ...
        if "fat_fraction" in snakemake.output:
...
elif hasattr(snakemake.input, "start_segmentation") and hasattr(snakemake.input, "end_segmentation"):
    # Set up base parameters for two-point mode
    params = {
        "muscle_segmentation_path": snakemake.input["muscle_segmentation"],
        "output_path": snakemake.output["cropped_muscle"],
    ...
    # Full volume mode - no reference segmentations
    params = {
        "muscle_segmentation_path": snakemake.input["muscle_segmentation"]
    }
